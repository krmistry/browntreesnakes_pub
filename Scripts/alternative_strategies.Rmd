...
-   50 ha area
    ...
    -   I assume that one team can do a visual survey OR check traps on 4 transects in 1 night, and one team can check bait tubes on 8 transects in 1 night
        -   One team could survey or check traps on all transects in 29 days (two teams could do it 15 days).
        -   One team could check all bait tubes in 15 days
...
3.  ADS plus monitoring

    -   The same ADS schedule as the ADS only strategy (2 drops per quarter), but with the addition of light monitoring via visual search so that it can be a dynamic strategy. Uses only one threshold to either turn ADS on or off.

    -   Estimate population every 4 quarters (annually)

4.  Eradication focus without ADS
...
num_replicates <- 50
replicates_strategy_one <- list()
replicates_strategy_one$low_more_small <- list()
for(var in 1:num_replicates) {
  results <- readRDS(file = here("Results", "alt_strategies", "Strategy_one", "low_more_small", paste0("strategy_one_IBM_start_pop_low-more_small-var_", var, ".rds")))
  replicates_strategy_one$low_more_small[[var]] <- results[[1]]
}
...
for(n in 1:num_replicates) {
  ...
  results <- replicates_strategy_one$low_more_small[[n]]$all_quarters
  num_quarters <- length(unique(results$Quarter))
  ...
  names(erad_plot_data_1[[n]]) <- c(1:num_quarters)
}
...
Parallel processing for strategy one:
...
# Using the parallel function, sequentially
for(variant in 1:10) {
...
### Strategy 3: ADS + monitoring

The same ADS schedule as strategy 1, but with light visual survey monitoring and threshold to stop and start ADS. This will allow to directly determine how much wasted effort the first strategy has, as well as giving an idea of how quickly the population might bounce back once ADS is stopped

-   Starting methods: one ADS treatment (2 drops) per quarter over entire area, and light effort visual surveys (every day for 2 weeks with 2 teams, in order to cover the entire area)

-   Thresholds:

    -   Estimated mean density is of upper size classes is \<= 1 snake/ha combined

        -   If met: in next 4 quarters, stop ADS and only perform visual surveys (same effort as initial methods)

-   Estimation occurs once per year
...
### Strategy 4: Eradication focus without ADS
...
-   Threshold 1: estimated mean density of extra-large size class is 0 for \>=2 quarters
    ...
    -   If met: stop all methods for 2 quarters, do 1 rounds of low effort visual survey (3 weeks, 3 teams) in 3rd and 4th quarters

-   Threshold 2: estimation mean density of extra-large size class increases above 0

    -   Return to initial methods
...
-   Transect details:
    ...
    -   Visual surveys occur every other day for 9 weeks (but only 3 weeks of that is the primary sampling period (the other 6 weeks overlap with bait tubes)
        ...
        -   This means that 1 team can cover the entire area each quarter with initial methods (some overlap would occur, but that wouldn't change the coverage parameter since 100% coverage is the maximum). It would change the encounter probability though - however, since 34 days is only 5 days more than 29, I'll operate as though encounter is based on just 1 team. Increasing effort, though, would require increasing the number of teams (and therefore the encounter probability), in the threshold 1 scenario

            -   Going with 100% coverage with 1 team (since they're spatially separated)

        -   For threshold 1 methods, there are only 10 days of sampling, so it would take 3 teams to cover the whole area once
...
```{r strategy_4_define_thresholds}
...
# Separating out condition 
methods <- method_options[[1]]$methods
coverage <- method_options[[1]]$erad_coverage
primary_sampling_period <- method_options[[1]]$primary_sampling_period
erad_quarters <- method_options[[1]]$erad_quarters
erad_days <- method_options[[1]]$erad_days
ADS_overlap <- method_options[[1]]$ADS_overlap_on_transect
num_teams <- method_options[[1]]$num_teams
...
```{r strategy_4_run_estimation_once}
...
## Testing to see if any thresholds are met
condition <- strat_4_threshold_fun(est_v_sim_N_plots$mean_N_df) 
```

```{r strategy_4_dynamic}
...
while(t <= final_time_step) {
  ...
  # Saving IBM observed and effort from this run
    ...
    # Save IBM results from this run
      saveRDS(IBM_results, file = here("Results", "alt_strategies", "Strategy_three", "IBM", paste0("IBM_results_test_IBM_set-", t, ".rds")))
  ...
  ## Testing to see if any thresholds are met
  mean_N_df <- est_v_sim_N_plots$mean_N_df
  condition <- strat_4_threshold_fun(mean_N_df)
  # Advancing counter by the number of quarters that have been estimated
  t <- t+1
}
