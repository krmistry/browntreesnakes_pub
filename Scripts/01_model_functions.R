...
daily_operations <- function(first_day_pop,
  ...
  # Sequence of events: growth, eradication if it occurs, natural mortality 
  for(day in 1:(total_days-1)) {
    ...
    # Natural mortality occurs on entire population every day
    mort_results <- daily_mortality_fun(pop, size_class_limits)
    # If the last snake dies due to natural mortality, stop the simulation run
    ...
    ## Each snake grows (or not) based on it's individual growth quantile, assuming the population still exists
    ...
    } else { # If no eradication is occurring at all, then re-combine the growing pop with surviving moms for 
      # the next day's population
      daily_pop[[day + 1]] <- rbind(growing_pop, surviving_moms)
    }
